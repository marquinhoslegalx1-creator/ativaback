...
app.post("/generate-image", async (req, res) => {
  try {
    ...
    const response = await fetch(
      // Atualizando o modelo para o recomendado para geração de imagens
    ...
    if (!response.ok) {
      ...
      console.error(`Gemini Image API retornou ${response.status}:`, errorBody);
      return res.status(500).json({ error: "Erro na API de imagem do Gemini", details: errorBody });
    }
    ...
    if (!base64) {
      console.log("Erro: Sem base64 na resposta. Resposta completa da API:", JSON.stringify(result, null, 2));
